class Balance::BaseCalculator
  ...
  private
    ...
    def flows_for_date(date)
      ...
      if account.balance_type == :non_cash && %w[Loan OtherAsset OtherLiability].include?(account.accountable_type)
    ...
    def derive_non_cash_balance(non_cash_balance, date, direction: :forward)
      entries = sync_cache.get_entries(date)
      # Loans, OtherAsset, and OtherLiability accounts track their balance via transactions
      if account.balance_type == :non_cash && %w[Loan OtherAsset OtherLiability].include?(account.accountable_type)
